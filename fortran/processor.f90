module processor
   ...
   private																	! private can only be accessed in the module
   public :: plot, input, output, plotdef, ploteval, &						! public can be accessed outside the module
             plotevec, ploteig, plotanim, numnode, &
             stopwatch, plotmatlabdef, plotmatlabeval, &
             plotmatlabevec, plotmatlabeig
...
!---------------------------------------------------------------------
   ...
   case( 'matlab' )
      ...
      case( 'eigenmode' )
            ...
            call plotmatlabeig(title,data1(1),data2,data3)
      case default
...
!  This subroutine reads in the input in ansys format.
   ...
   integer :: i, j, e, ixsave, eixsave(4)
   ...
   do
      ...
      elseif (command == 'ET' .or. command == 'et') then
         ...
         elseif (cvalue == 'PLANE42RECT' .or. cvalue == 'plane42rect') then
            ename(ivalue) = 'plane42'
          elseif (cvalue == 'SHELL41' .or. cvalue == 'shell41') then ! changed
            ename(ivalue) = 'shell41'
         elseif (cvalue == 'SHELL41RECT' .or. cvalue == 'shell41rect') then 
            ename(ivalue) = 'shell41'           
         else
      ...
      elseif (command == 'R' .or. command == 'r') then
         ...
         elseif (ename(currentet) == 'shell41') then		!changed
            ! define 4-noded quad element thickness
            mprop(ivalue)%thk = rvalue            
         else
            write (*, *)
      ...
      elseif (command == 'D' .or. command == 'd') then
         ...
         bound(nb, 1) = ivalue
         if (cvalue == 'UZ' .or. cvalue == 'uz') then			! changed
            bound(nb, 2) = 1
         elseif (cvalue == 'UX' .or. cvalue == 'ux') then 		! changed
            bound(nb, 2) = 2
         elseif (cvalue == 'UY' .or. cvalue == 'uy') then		! changed
            bound(nb, 2) = 3            
         else
            write (*, *) 'Error: Nodal dof not "uw", "ux" or "uy"'
            stop
         endif
         bound(nb, 3) = rvalue
      elseif (command == 'F' .or. command == 'f') then
         backspace (10)
         np = np + 1
         read (10, *) command, ivalue, cvalue, rvalue, ivalue2
         loads(np, 1) = 1
         loads(np, 2) = ivalue
         if (cvalue == 'FZ' .or. cvalue == 'fz') then
            loads(np, 3) = 1
...
!      ier = pgopen(' ')
   ...
   ! Must call pgeras before defining color-map (GrWin specific)
   call pgeras
...
! Plot vector field
   ...
   real(4) :: xmin, xmax, ymin, ymax, lx, ly, lmax, clx, cly, clmax
   real(4) :: px(5), py(5)
   real(8) :: maxep1, avepx, avepy, ca, sa, sf
   integer :: ier, pgopen
   integer :: i, j, e, nen
...
!      trim(filename)//'_03.ps'
   ...
   do e = 1,ne
      ...
      ca = cos(-ang(e))
      sa = sin(-ang(e))
      px(1) = real(avepx - 0.5 * abs(p1(e)) * ca/sf)
      py(1) = real(avepy - 0.5 * abs(p1(e)) * sa/sf)
      px(2) = real(avepx + 0.5 * abs(p1(e)) * ca/sf)
      py(2) = real(avepy + 0.5 * abs(p1(e)) * sa/sf)
      if (p1(e) >= 0.) then
         call pgsci(4)
      else
         call pgsci(2)
      ...
      ca = cos(-ang(e)+pi/2.)
      sa = sin(-ang(e)+pi/2.)
      px(1) = real(avepx - 0.5 * abs(p2(e)) * ca/sf)
      py(1) = real(avepy - 0.5 * abs(p2(e)) * sa/sf)
      px(2) = real(avepx + 0.5 * abs(p2(e)) * ca/sf)
      py(2) = real(avepy + 0.5 * abs(p2(e)) * sa/sf)
      if (p2(e) >= 0.) then
         call pgsci(4)
      else
         call pgsci(2)
...
! Note that the library only accepts single precision real variables.
   ...
   real(4) :: xmin, xmax, ymin, ymax, lx, ly, lmax
   real(4) :: px(5), py(5)
   integer :: ier, pgopen
...
! eval(:)  real(8)  element values
   ...
   real(4) :: px(5), py(5)
   real(8) :: avepx, avepy, ca, sa, sf
   integer :: pgopen, cid, ncid, ncidmin, ncidmax
   integer :: i, j, e, nen
   integer :: panel(8)
   integer :: strnglen
...
! Add space around structure
      ...
      call pgsvp(0.,.96,0.,.96)
      if (legend) then
         call pgswin(-0.1*lmax(window)+xmin, 2.0*lmax(window)+xmin, &
                     -0.1*lmax(window)+ymin, 2.0*lmax(window)+ymin)
      else
